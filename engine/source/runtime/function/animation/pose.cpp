    ...
    for (int i = 0; i < m_bone_poses.size(); i++)
    {
        ...
        if (sum_weight != 0)
        {
            ...
            bone_trans_one.m_position  = Vector3::lerp(
                bone_trans_one.m_position, bone_trans_two.m_position, (sum_weight - cur_weight) / sum_weight);
            bone_trans_one.m_scale =
                Vector3::lerp(bone_trans_one.m_scale, bone_trans_two.m_scale, (sum_weight - cur_weight) / sum_weight);
            bone_trans_one.m_rotation = Quaternion::sLerp(
                (sum_weight - cur_weight) / sum_weight, bone_trans_one.m_rotation, bone_trans_two.m_rotation, true);
